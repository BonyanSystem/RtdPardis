...
public final class JettyClient {
    ...
    public String getToken(EventRecord eventRecord) {
        String token = "";
        this.nodeLogger.info("getToken...");
        this.method = Utils.getErField(eventRecord, "Token-Method");
        this.nodeLogger.info("1.4getToken..." + this.method);
        this.requestId = Utils.getErField(eventRecord, "Token-Request-Id");
        this.remoteAddress = Utils.getErField(eventRecord, "Token-Remote-Address");

        try {
            this.remotePort = Utils.getErFieldInt(eventRecord, "Token-Remote-Port");
        ...
        this.remoteScheme = Utils.getErField(eventRecord, "Token-Remote-Scheme");
        this.requestURI = Utils.getErField(eventRecord, "Token-Request-Uri");
        this.queryString = Utils.getErField(eventRecord, "Token-Query-String");
        this.nodeApplication.associateTransaction(eventRecord, this.requestId);
        this.errMsg = this.validateInputFields();
        if (!this.errMsg.isEmpty()) {
            this.nodeLogger.error(this.errMsg);
    ...
    private int sendHttpRequest(Request request, final AtomicReference<Result> resultRef, EventRecord eventRecord) {
        ...
        Field responseBlock = eventRecord.addField("Response___");
